      ...
      <form class="card" [formGroup]="form" (ngSubmit)="submit()">
        ...
        <div class="row">
          <mat-form-field appearance="outline">
            <mat-label>{{ i18n.t('auth.firstName') }}</mat-label>
            <input matInput formControlName="prenom" />
          </mat-form-field>
          <mat-form-field appearance="outline">
            <mat-label>{{ i18n.t('auth.lastName') }}</mat-label>
        ...
        <mat-form-field appearance="outline">
          <mat-label>{{ i18n.t('auth.phone') }}</mat-label>
          <input matInput formControlName="telephone" placeholder="+237 6XX XXX XXX" />
        </mat-form-field>

        <mat-form-field appearance="outline">
          <mat-label>{{ i18n.t('auth.password') }}</mat-label>
          <input matInput [type]="showPassword() ? 'text' : 'password'" formControlName="motDePasse" autocomplete="new-password" />
        ...
        <mat-form-field appearance="outline">
          <mat-label>{{ i18n.t('auth.confirmPassword') }}</mat-label>
        ...
        @if (form.errors?.['mismatch'] && form.get('confirmerMotDePasse')?.touched) {
        ...
        <p class="hint">{{ i18n.t('auth.alreadyRegistered') }} <a routerLink="/connexion">{{ i18n.t('auth.login') }}</a></p>
      </form>
    </div>
    <app-site-footer />
  `,
...
export class RegisterComponent {
  ...
  readonly form = this.fb.nonNullable.group(
    ...
    { validators: passwordsMatchValidator('motDePasse', 'confirmerMotDePasse') },
  );
  ...
  async submit(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const { confirmerMotDePasse, ...payload } = this.form.getRawValue();
      await this.authService.register(payload);
      this.router.navigateByUrl('/proprietaire/tableau-de-bord');
    } catch (err) {
      this.error.set(describeAuthError(err, this.i18n.t('auth.registerError')));
    } finally {
